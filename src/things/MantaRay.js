...
import {  Vector3, PointsMaterial, CatmullRomCurve3, TubeBufferGeometry } from 'three';
import { rgbTo01, getRandomFloat } from '../helper';
import { OBJLoader } from 'three/examples/jsm/loaders/OBJLoader';
const MANTA_ANIMATION_SPEED = 0.7;
const SPEED = 5000;
...
export function MantaRay(props) {
  ...
  useEffect(() => {
    ...
    animationAction.setEffectiveTimeScale(MANTA_ANIMATION_SPEED).play();

    const posArray = mantaPathGeometry.attributes.position.array;

    for (let i = 0; i < posArray.length; i = i + 3) {
      v3Array.push(
        new Vector3(posArray[i + 0], posArray[i + 1], posArray[i + 2])
      );
    }
  });
  ...
  const spline = useMemo(() => {
    const curve = new CatmullRomCurve3(v3Array);
    curve.curveType = 'centripetal';
    curve.closed = true;
    // WTF?
    tubeGeom= new TubeBufferGeometry(spline, 250, 0.2, 10, true);
